import { loadingMessages, messages, notifications, sessStore, user } from './stores';
import { get } from 'svelte/store';
import type { Message, MessageCache } from './types';
...
export async function getMessages(
	...
	const messagesCache: MessageCache = get(messages);
	const userStore = get(user);
	const channelId = params.channelId ? params.channelId : params.id;
	const offset = params.offset || 0;
	const limit = params.limit || 25;
	if (!channelId) return [];
	if (messagesCache && messagesCache[channelId] && !loadMore) {
